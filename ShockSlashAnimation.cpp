...
ShockSlashAnimation::ShockSlashAnimation(const Vector &position) {
...
ShockSlashAnimation::~ShockSlashAnimation() {
...
std::list<Particle*> ShockSlashAnimation::getParticles() {
...
bool ShockSlashAnimation::hasFinished() {
    bool finished = true;
...
bool ShockSlashAnimation::isBlocking() {
    return false;
}

void ShockSlashAnimation::update(double dt) {
    lifetime += dt;
    ...
    while (newParticleCounter > interval) {
        ...
        Particle *particle = new ShockParticle(worldPosition, position);
